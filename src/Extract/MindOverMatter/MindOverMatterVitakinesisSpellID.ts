...
export const ExtractDefineMindOverMatterVitakinesisSpellIDList = [
    ...
    "vita_sleeping_trance"        , // [Ψ]清醒休憩 你可以用冥想来代替睡眠。只要你保持冥想，你就很少再需要花一夜的时间睡觉了。
    "vita_cure_disease"           , // [Ψ]免疫增强(C) 将你的免疫系统加速，以对抗你可能正在遭受的任何疾病。此项灵能<color_yellow>需要专注维持</color>，可能<color_red>失效</color>于<color_yellow>打断专注</color>后。
    "vita_stop_infection"         , // [Ψ]消除感染 之前你用一个念头就能止血。现在，你可以用你的灵能对伤口消毒或治疗感染。
    "vita_healing_trance"         , // [Ψ]再生冥想 进入一种恍惚的状态，以数倍于自然恢复的速度治愈你的伤口。
    "vita_attack_touch"           , // [Ψ]撕裂触碰 疗愈者灵能的反向应用，你随机撕裂肌肉、切断血管，对目标造成显著的伤害。
    "vita_purge_rads"             , // [Ψ]辐射净化 使用你的灵能来隔离和冲洗你体内的辐射。根据你的辐射程度，可能会有严重的副作用。
    "vita_blood_purge"            , // [Ψ]血液滤清 你将你的思想浸入你的血液中，并清除你在那里发现的杂质。
...
export type ExtractDefineMindOverMatterVitakinesisSpellID = typeof ExtractDefineMindOverMatterVitakinesisSpellIDList[number];