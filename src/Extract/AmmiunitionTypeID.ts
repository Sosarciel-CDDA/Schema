/**从文件提取的预定义的AmmiunitionTypeID 列表*/
export const ExtractDefineAmmiunitionTypeIDList = [
    "458wm"                       , // .458 温彻斯特马格南弹
    "50"                          , // .50 BMG 弹
    "4570"                        , // .45-70 弹
    "50ae"                        , // .50 AE弹
    "50beowulf"                   , // 12.7x42mm
    "450"                         , // .450 大毒蛇弹
    "338lapua"                    , // .338 拉普阿弹
    "77mm_arisaka"                , // 7.7x58mm 有阪弹
    "nail"                        , // 钉子
    "nuts_bolts"                  , // 螺栓螺母
    "barb"                        , // 毒刺
    "gene_sting"                  , // 尤格毒刺
    "nether_huntsman_javelin_ammo", // 异界组织
    "BB"                          , // BB弹
    "bolt_ballista"               , // 弩炮炮矢
    "bolt"                        , // 弩矢
    "arrow"                       , // 箭
    "strange_arrow"               , // 奇异箭
    "atlatl"                      , // 梭镖投射器用矛
    "pebble"                      , // 小石子
    "rock"                        , // 石头
    "shot"                        , // 霰弹
    "410shot"                     , // .410
    "blunderbuss"                 , // 火铳
    "22"                          , // .22 LR 弹
    "powder_blank"                , // 空包弹
    "9mm"                         , // 9x19mm 弹
    "357sig"                      , // .357 西格弹
    "357mag"                      , // .357 马格南弹
    "9x18"                        , // 9x18mm
    "380"                         , // .380 ACP 弹
    "38"                          , // .38 特制弹
    "38super"                     , // .38 超级弹
    "40"                          , // .40 S&W 弹
    "10mm"                        , // 10mm Auto 弹
    "44"                          , // .44 马格南弹
    "45"                          , // .45 ACP弹
    "454"                         , // .454 卡苏尔弹
    "460sw"                       , // .460 S&W 马格南弹
    "45colt"                      , // .45 柯尔特弹
    "500"                         , // .500 Magnum
    "57"                          , // 5.7mm
    "46"                          , // 4.6mm
    "762"                         , // 7.62x39mm 弹
    "545x39"                      , // 5.45x39mm 弹
    "223"                         , // .223 弹
    "303"                         , // .303 英式弹
    "3006"                        , // .30-06 弹
    "30carbine"                   , // .30 卡宾枪弹
    "270win"                      , // .270 温彻斯特弹
    "308"                         , // .308 弹
    "40x46mm"                     , // 40x46mm 榴弹
    "40x53mm"                     , // 40x53mm 榴弹
    "66mm"                        , // high-explosive anti-tank warhead
    "120mm"                       , // high-explosive anti-tank warhead
    "84x246mm"                    , // 84mm recoilless projectile
    "m235"                        , // M235 66mm 燃烧火箭弹
    "mortar_60mm"                 , // 60mm 迫击炮炮弹
    "sling-ready_grenade"         , // 投射用手雷
    "gasoline"                    , // 汽油
    "jp8"                         , // jp8
    "avgas"                       , // 航空燃油
    "diesel"                      , // 柴油
    "lamp_oil"                    , // 煤油
    "crude_lamp_oil"              , // 自制灯油
    "butane"                      , // 丁烷
    "aerosol_paint"               , // 喷漆
    "permanent_ink"               , // 油墨
    "candle_wax"                  , // 蜡块
    "flare_nitrate"               , // 照明剂
    "match"                       , // 火柴
    "incense"                     , // 熏香
    "oxygen"                      , // 氧气
    "hydrogen"                    , // 氢气
    "nitrogen"                    , // 氮气
    "motor_oil"                   , // 机油
    "crude_oil"                   , // 原油
    "thread"                      , // 棉线
    "anesthetic"                  , // 麻醉剂
    "soap"                        , // 肥皂
    "tape"                        , // 强力胶带
    "superglue"                   , // super glue
    "battery"                     , // kJ
    "money"                       , // 美分
    "plutonium"                   , // 钚
    "uranium"                     , // 铀
    "reactor_slurry"              , // 钚浆
    "12mm"                        , // 12mm slugs
    "plasma"                      , // 固态氢
    "flammable"                   , // 可燃液体
    "weldgas"                     , // 焊接气体
    "propane"                     , // 丙烷
    "biogas"                      , // 生物燃气
    "conc_alcohol"                , // 浓缩酒精
    "methanol_fuelcell"           , // 甲醇
    "8x40mm"                      , // 8x40mm 无壳弹
    "20x66mm"                     , // 20x66mm 无壳霰弹
    "signal_flare"                , // 信号弹
    "charcoal"                    , // 木炭
    "coal"                        , // 煤炭
    "thrown"                      , // 投掷武器
    "ampoule"                     , // 医用安瓿
    "albuterol"                   , // 舒喘宁
    "liq_bandage"                 , // 液体绷带
    "stimpack_ammo"               , // 兴奋剂
    "components"                  , // 组件
    "rebreather_filter"           , // 呼吸器二氧化碳吸收物
    "rebreather_o2_regenerator"   , // 呼吸器氧气再生物
    "gasfilter_s"                 , // 过滤面罩滤芯
    "gasfilter_m"                 , // 防毒面具滤毒罐
    "300"                         , // .300 温彻斯特马格南弹
    "32"                          , // .32 ACP 弹
    "762x25"                      , // 7.62x25mm
    "flintlock"                   , // 纸卷弹
    "36paper"                     , // .36 cap & ball
    "44paper"                     , // .44 雷管弹
    "762R"                        , // 7.62x54mmR 弹
    "fishspear"                   , // 枪矛
    "fish_bait"                   , // 鱼饵
    "homebrew_rocket"             , // 自制火箭
    "atgm"                        , // 152mm 反坦克导弹
    "unfinished_char"             , // 半焦燃料
    "chemical_spray"              , // 喷雾式化学药剂
    "milk"                        , // 奶
    "milk_raw"                    , // 生乳
    "paintball"                   , // 彩弹
    "nitrox"                      , // 压缩空气
    "paper"                       , // 纸
    "extinguishing_agent"         , // 灭火剂
    "teargas_spray"               , // 催泪瓦斯
    "tinder"                      , // 火绒
    "esbit"                       , // 乌洛托品
    "water"                       , // 水
    "300blk"                      , // .300 BLK弹
    "123ln"                       , // 12.3ln 弹
    "273x110"                     , // 27.3x110 Diyua
    "273x44"                      , // 27.3x44 Diyua
    "273x44RG"                    , // 27.3x44 Diyua Grenade
    "33naval"                     , // 3.3 海军弹
    "caotel_cell"                 , // 卡特尔单元
    "khuunaofaai_payload"         , // 锟厄焚烟花载荷
    "khuunaofaai_payload_fire"    , // 锟厄焚军械炮燃烧载荷
    "khuunaofaai_payload_acid"    , // 锟厄焚军械炮腐蚀载荷
    "khuunaofaai_payload_glue"    , // 锟厄焚军械炮黏着载荷
    "glowstick_juice"             , // 荧光液
    "cannon"                      , // 加农炮弹
    "graphite"                    , // 石墨
    "quill_ink"                   , // 羽毛笔墨水
    "black_pen_ink"               , // 黑色墨水
    "red_pen_ink"                 , // 红色墨水
    "blue_pen_ink"                , // 蓝色墨水
    "green_pen_ink"               , // 绿色墨水
    "ammonia_liquid"              , // 氨
    "notch"                       , // 火种凹槽
    "turpentine"                  , // 松节油
    "airgun_pellet"               , // 气枪子弹
] as const;
/**从文件提取的预定义的AmmiunitionTypeID */
export type ExtractDefineAmmiunitionTypeID = typeof ExtractDefineAmmiunitionTypeIDList[number];